 ...
 *  On input:
 *	s   is the address of a pointer to a string whose ending is at
 *	    'send' - 1.  The string begins with 'o', and the previous character
 *	    was a backslash.  At exit, *s will be advanced to the byte just
 *	    after those absorbed by this function.  Hence the caller can
 *	    continue parsing from there.  In the case of an error, this routine
 *	    has generally positioned *s to point just to the right of the first
 *	    bad spot, so that a message that has a "<--" to mark the spot will
 ...
 *  On input:
 *	s   is the address of a pointer to a string whose ending is at
 *	    'send' - 1.  The string begins with 'x', and the previous character
 *	    was a backslash.  At exit, *s will be advanced to the byte just
 *	    after those absorbed by this function.  Hence the caller can
 *	    continue parsing from there.  In the case of an error, this routine
 *	    has generally positioned *s to point just to the right of the first
